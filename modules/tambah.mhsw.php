...
function DftrUsr() {
  ...
  echo "<p><table class=box cellspacing=1 cellpadding=4>
    ...
    <input type=hidden name='mnux' value='daft.mhsw'>
...
function DaftarUsr() {
  if (!empty($_SESSION['LevelID'])) {
    ...
    $a = "<p><table class=box cellspacing=1 cellpadding=4>
      <tr><td class=ul colspan=5><a href='?mnux=daft.mhsw&gos=UsrEdt&LevelID=$_SESSION[LevelID]&md=1'></a></td></tr>";
    while ($w = _fetch_array($r)) {
      $n++;
      $c = ($w['NA'] == 'Y')? 'class=nac' : 'class=ul';
      $a .= "<tr><td $c>$n</td>
        <td $c><a href='?mnux=daft.mhsw&gos=UsrEdt&LevelID=$_SESSION[LevelID]&md=0&Lgn=$w[Login]'><img src='img/edit.png' border=0>
        $w[Login]</a></td>
        <td $c>$w[Nama]</td>
        <td $c>$w[ProdiID]&nbsp;</td>
        <td $c align=center><img src='img/book$w[NA].gif'></td>
				<td $c align=center><a href='?mnux=daft.mhsw&gos=dltUsrCek&LevelID=$_SESSION[LevelID]&Lgn=$w[Login]'><img src='img/del.gif'></td>
        </tr>";
    }
    return $a ."</table></p>";
  }
...
function UsrEdt() {
  ...
  <tr><td colspan=2><input type=submit name='Simpan' value='Simpan'>
    ...
    <input type=button name='Batal' value='Batal' onClick=\"location='?mnux=daft.mhsw'\"></td></tr>
...
function dltUsrCek(){
	...
	echo Konfirmasi("Delete User", "Yakin Anda ingin menghapus user : <br />
									...
									<input type=button name='hapus' value='Delete User' onClick=\"location='?mnux=daft.mhsw&gos=dltUsr&Lgn=$Login'\">");
}
function dltUsr(){
