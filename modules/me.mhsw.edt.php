...
function TampilkanHeaderMhsw($w) {
  ...
  <tr><td class=ul>Pilihan</td><td class=ul><a href='?mnux=mhsw'>Kembali ke Daftar Mhsw</a> |
    <a href='?mnux=modules/mhsw.foto&mhswid=$w[MhswID]'>Ganti Foto</a>
    </td></tr>
  </table>";
}
function pri() {
  include_once "modules/mhsw.edt.pri.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmPribadi';
  $sub();
}
function almt() {
  include_once "modules/mhsw.edt.almt.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmAlamat';
  $sub();
}
function akd() {
  include_once "modules/mhsw.edt.akd.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmAkademik';
  $sub();
}
function sek() {
  include_once "modules/mhsw.edt.sek.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmSekolah';
  $sub();
}
function ortu() {
  include_once "modules/mhsw.edt.ortu.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmOrtu';
  $sub();
}
function bank() {
  include_once "modules/mhsw.edt.bank.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmBank';
  $sub();
}
function pt() {
  include_once "modules/mhsw.edt.pt.php";
  $sub = (!empty($_REQUEST['sub']))? $_REQUEST['sub'] : 'frmPT';
  $sub();
}

function mb(){
	include_once "modules/mhsw.edt.masterbipot.php";
