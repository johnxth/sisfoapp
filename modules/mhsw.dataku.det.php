...
include_once "modules/mhsw.dataku.hdr.php";
...
function dataMhswPribadi($m) {
  ...
  <tr><td class=ul colspan=2><b>Alamat Tinggal di Bandung</b></td></tr>
...
function TuliskanScriptLihatKeu() {
  ...
  function LihatKeu(ID) {
    lnk = "modules/mhsw.dataku.det.popup.php?KHSID="+ID;
...
// *** Sub Menu ***
$arrMenudataMhsw = array(
  "Data Pribadi->dataMhswPribadi",
  "Orang Tua->dataMhswOrtu",
  "Data Akademik->dataMhswAkademik",
  "History Semester->dataMhswSemester",
  "History Matakuliah->dataMhswMK",
  "Prestasi & Wanprestasi->dataMhswPres",
  "Tugas Akhir->dataTugasAkhir",
  "Keuangan->dataMhswKeuangan");

// *** Parameters ***
$mhswid = GetSetVar('mhswid');
$UkuranHeader = GetSetVar('UkuranHeader', 'Besar');
$dataMhsw = GetSetVar('dataMhsw', 'dataMhswPribadi');
$gos = empty($_REQUEST['gos'])? 'dataMhswPribadi' : $_REQUEST['gos'];

// *** Main ***
TampilkanJudul("Detail Mahasiswa");
if (!empty($_SESSION['_Login'])) {
  $m = MhswDet();  
  TampilkanSubMenu('modules/mhsw.dataku.det', $arrMenudataMhsw, 'dataMhsw', $dataMhsw);
  $dataMhsw($m);
}
?>
