...
function DaftarGolongan() {
  ...
  $r = _query($s); $n = 0;
  $count = _num_rows($r);
  if ($count == 0) echo "<a href='?mnux=modules/golongan&gos=GolImprt'>Import Default Golongan</a>";
  ...
  while ($w = _fetch_array($r)) {
    ...
    echo "<tr>
    <td class=inp>$n</td>
    <td class=ul><a href='?mnux=modules/golongan&md=0&gos=GolEdt&_GID=$w[GolonganID]&_KID=$w[KategoriID]'><img src='img/edit.png'></a></td>
...
function GolImprt(){
  ...
  echo Konfirmasi("Import Data Golongan", "Anda akan mengimport data default untuk Master Golongan pada Prodi <b>$w[Nama]</b>. Proses ini hanya akan mengimport data default. Silakan edit kembali nilai nominal uang sesuai dengan kebijakan di institusi anda.
                  <hr><a href=?mnux=modules/golongan&gos=DaftarGolongan>Batal</a> | <a href='?mnux=modules/golongan&gos=doImport'>Proses</a>");
}
...
function GolEdt() {
  ...
  <input type=hidden name='mnux' value='modules/golongan'>
...
function GolSav() {
  ...
  $md = $_REQUEST['md']+0;
  $balik = "<script>window.location='?mnux=modules/golongan';</script>";
  if ($md == 0) {
    $s = "update golongan
