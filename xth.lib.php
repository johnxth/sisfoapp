...
function Konfirmasi($jdl, $isi) {
  ...
  <tr><td align=center>$isi</td></tr></table></p>";
}
function Konfirmasi2($jdl, $isi) {
  Return "<p><table class=boxlogin cellspacing=1 cellpadding=2 align=center>
    <tr><td>$isi</td></tr></table></p>";
}
...
function ErrorMsg($jdl='Error', $isi='') {
  Return "<p><table class=box cellspacing=1 cellpadding=4 width=50%>
...
function TampilkanFilexthPRN($nmf, $mnux='', $ttp=1) {
  ...
  echo "<p><a href='xthprn.php?f=$nmf'>Cetak Laporan</a> | $_mnux $_ttp";
  $f = fopen($nmf, "r");
...
function TampilkanTahunProdiProgram($mnux='', $gos='', $pref='', $token='', $JarakNPM=0) {
  ...
  $optprg = GetOption2('program', "concat(ProgramID, ' - ', Nama)", 'ProgramID', $_SESSION['prid'], "KodeID='$arrID[Kode]'", 'ProgramID');
  if ($JarakNPM == 1) {
    $_npm = "<tr><td class=inp1>Dari NIM</td>
	...
	$Autosubmit = ($JarakNPM == 1) ? " onChange='this.form.submit()'" : '';
...
function TampilkanPencarianMhsw($mnux='mhswakd', $gos='CariMhsw', $btn=2) {
  global $arrID;
  $strbtn = "<input type=submit name='crmhsw' value='NIM'>";
