...
function PMBEdt($mnux='pmbform.edt', $pmbgos='PMBSav') {
  ...
  $md = $_REQUEST['md']+0;
  if ($md == 0) {
    $w = GetFields('pmbweb', 'PMBID', $_REQUEST['pmbid'], '*');
  ...
  $_Program = GetOption2('program', "concat(ProgramID, ' - ', Nama)", 'Nama', $_REQUEST['prgid'], "KodeID='UKRIDA'", 'ProgramID');
...
function PMBSav($gos='') {
  ...
  //$BeliFormulir = GetaField('statusawal', 'StatusAwalID', $StatusAwalID, 'BeliFormulir');
  ...
  elseif ($md == 1) {
    ...
    $r = _query($s);
    $bayar = GetaField('pmbformulir','PMBFormulirID',$_REQUEST['PMBFormulirID'],'Harga');
    $bayar = number_format($bayar);
    $prod = GetaField("prodi",'ProdiID',$_REQUEST['Pilihan1'],'Nama');
    $lambat = GetaField('pmbperiod','PMBPeriodID',$_SESSION['pmbaktif'],'BayarSelesai');
    $lambat = FormatTanggal($lambat,'-');
    echo Konfirmasi("Konfirmasi","<p>Anda telah terdaftar di UKRIDA pada Program Studi <b>$prod</b><br>Nomor registrasi Anda adalah : <b>$PMBID</b> <font color=Red>(harap dicatat untuk konfirmasi)</font></p>Silakan melakukan 
           pembayaran registrasi senilai <b>Rp. $bayar</b> paling lambat tanggal : <b>$lambat</b><br><br>
           Informasi lebih lanjut hubungi:<br>Bagian Admisi<br>Lantai I Gedung A Kampus I Jl. Tanjung Duren Raya No. 4<br>Jakarta Barat<br>Telepon : +62 (021) 5666952<br>Fax : +62 (021) 566651/56");
  }
  else {
    $strStatusAwalID = GetaField('statusawal', 'StatusAwalID', $StatusAwalID, 'Nama');
    echo Konfirmasi("Formulir Pendaftaran Tidak Ditemukan",
